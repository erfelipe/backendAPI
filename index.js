...
app.use(cors()); //permitir conexoes locais
app.use(bodyparser.json());//entrada da api

var conString = config.urlConnection;

var client = new Client(conString); //ligacao com user e pass no postgree
    ...
    client.query(
      "DELETE FROM Usuarios WHERE id = $1", [req.params.id], (err, result) => {
        if (err) {
          ...
          if (result.rowCount == 0) {
            res.status(404).json({ info: "Registro " + req.params.id + " não encontrado."  });
          } else {
